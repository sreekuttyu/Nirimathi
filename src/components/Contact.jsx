...
function InputField({ label, type = 'text', name, placeholder, required, as }) {
  ...
  const baseStyle = {
    ...
    color: '#F6F2EA', background: 'transparent', outline: 'none', width: '100%',
    padding: '0.75rem 0',
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label style={{ fontFamily: 'var(--font-sans)', fontSize: '0.6rem', fontWeight: 500,
        letterSpacing: '0.34em', textTransform: 'uppercase',
        color: focused ? '#C9B79C' : 'rgba(246,242,234,0.4)', transition: 'color 0.3s' }}>
        {label}{required && <span style={{ color: '#8A6A45', marginLeft: 2 }}>*</span>}
      </label>
      {as === 'textarea'
...
export default function Contact() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      /* Form & info panel slide up */
      gsap.fromTo([formRef.current, infoRef.current],
        { y: 50, opacity: 0 },
        {
          y: 0, opacity: 1, stagger: 0.18, duration: 1.2, ease: 'expo.out',
  ...
  return (
    <section id="contact" ref={sectionRef}
      className="relative py-28 md:py-40 overflow-hidden" style={{ background: '#1F2B24' }}>

      <div className="absolute pointer-events-none" style={{
        width: 600, height: 600, left: '-10%', bottom: '-10%',
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-10 lg:gap-16">
          ...
          {/* Form */}
          <div ref={formRef} className="glass rounded-2xl p-8 md:p-12"
            style={{ boxShadow: '0 40px 80px -20px rgba(0,0,0,0.3)', opacity: 0 }}>
            {submitted ? (
              <motion.div className="flex flex-col items-center justify-center text-center py-12"
                ...
                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', fontWeight: 400, color: '#F6F2EA', marginBottom: '0.75rem' }}>
                  Message Received
                </h3>
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="flex flex-col gap-8">
                ...
                <InputField label="Your Vision" name="message"
                  placeholder="Tell us about your project — location, scale, timeline, and what matters most to you..." as="textarea" />
                <button type="submit"
                  className="group self-start flex items-center gap-3 px-8 py-4 rounded-full"
                  style={{ fontFamily: 'var(--font-sans)', fontSize: '0.68rem', fontWeight: 500,
                    letterSpacing: '0.22em', textTransform: 'uppercase',
                    background: '#8A6A45', color: '#F6F2EA', transition: 'background 0.3s' }}
          ...
          <div ref={infoRef} className="flex flex-col gap-8" style={{ opacity: 0 }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.5rem', fontWeight: 400,
              color: '#F6F2EA', letterSpacing: '-0.01em' }}>Nirmiti Studio</h3>
                ...
                <div key={label} className="flex gap-4">
                  ...
                  <div>
                    <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.6rem', fontWeight: 500,
                      ...
                      color: 'rgba(246,242,234,0.35)', marginBottom: '0.25rem' }}>{label}</div>
                    <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 300, fontSize: '0.85rem',
                      color: 'rgba(246,242,234,0.65)', whiteSpace: 'pre-line', lineHeight: 1.7 }}>{value}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* Map */}
            <div className="overflow-hidden rounded-xl" style={{ height: 220, border: '1px solid rgba(255,255,255,0.08)' }}>
              <iframe title="Nirmiti Studio Location" width="100%" height="100%"
            ...
            <div className="glass rounded-xl p-5">
              <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.6rem', fontWeight: 500,
                letterSpacing: '0.32em', textTransform: 'uppercase',
                color: 'rgba(246,242,234,0.35)', marginBottom: '0.75rem' }}>Studio Hours</div>
              {[['Mon – Fri','9:00 am – 6:00 pm'],['Saturday','10:00 am – 2:00 pm'],['Sunday','Closed']].map(([d,t],i) => (
                <div key={d} className="flex justify-between text-sm mt-1" style={{
                  fontFamily: 'var(--font-sans)', fontWeight: 300,
                  color: i === 2 ? 'rgba(246,242,234,0.28)' : 'rgba(246,242,234,0.55)',
                }}><span>{d}</span><span>{t}</span></div>
              ))}
            </div>
          </div>
        </div>
